...
extendPath hop locs (Path pd pl@(l@(Loc name (x,y)):_)) 
  = map updatePath nextLocs
  where nextLocs = filter (not . (`elem` pl)) $ within hop l locs  
        updatePath l' = Path (pd + distance l l') (l':pl)
...
findPath hop locs start end = bestFirstSearch goal succ cost startPath
  where startPath = Path 0 [start]
        goal = (== end) . head . pathLocs
        succ = extendPath hop locs
        cost :: Path -> Double
        cost p = distance (head $ pathLocs p) end + pathDist p
